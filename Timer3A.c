...
// Use TIMER3 in periodic mode to request interrupts at a particular
// period.
// Daniel Valvano
// September 11, 2013
...
// Outputs: none
void Timer3A_Init(void(*task)(void), uint32_t period){long sr;
  sr = StartCritical(); 
  SYSCTL_RCGCTIMER_R |= 0x08;   // 0) activate TIMER3
  PeriodicTask3 = task;          // user function
  TIMER3_CTL_R = 0x00000000;    // 1) disable TIMER3 during setup
  TIMER3_CFG_R = 0x00000000;    // 2) configure for 32-bit mode
  TIMER3_TAMR_R = 0x0000000;   // 3) configure for periodic mode, default down-count settings
  TIMER3_TAILR_R = period-1;    // 4) reload value
  TIMER3_TAPR_R = 0;            // 5) bus clock resolution
  TIMER3_ICR_R = 0x00000001;    // 6) clear TIMER3 timeout flag
  TIMER3_IMR_R = 0x00000001;    // 7) arm timeout interrupt
  NVIC_PRI8_R = (NVIC_PRI8_R&0x00FFFFFF)|0xB0000000; // 8) priority 6
  NVIC_EN1_R = 1<<3;           // 9) enable IRQ 35 in NVIC
  TIMER3_CTL_R = 0x00000001;    // 10) enable TIMER3
  EndCritical(sr);
}

void Timer3A_Disable(){
  TIMER3_CTL_R = 0x00000000;    // 10) disable timer3
}

void Timer3A_Enable(){
	TIMER3_CTL_R = 0x00000001;     // 9) enable timer
}

void Timer3A_Handler(void){
  TIMER3_ICR_R = TIMER_ICR_TATOCINT; // acknowledge TIMER3 timeout
