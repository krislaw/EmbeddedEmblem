...
void ST7735_sDecOut2(int32_t n){
	...
	int remainder = Abs(n);
	if(remainder > 999){
		ST7735_OutChar((remainder/1000) + '0');
		remainder = remainder % 1000;
	}
	ST7735_OutChar((remainder/100) + '0');
	remainder = remainder % 100;
	ST7735_OutChar('.');
	ST7735_OutChar((remainder/10) + '0');
	remainder = remainder % 10;
	ST7735_OutChar(remainder + '0');
...
void ST7735_uBinOut6(uint32_t n){
	...
	uint32_t fixed = (((n * 1000) >> 6) + 5) / 10; 	// convert to fixed floating point number
	
	//printf("FIXED: %d", fixed);
	
	uint32_t prevDigit = 0;
	
	for(uint32_t i = 10000; i > 0; i = i / 10){
		uint32_t digit = fixed / i;
		if(i == 10){
			ST7735_OutChar('.');
		}
		
		if((digit == 0) & (i > 100) & (prevDigit == 0)){
			ST7735_OutChar(' ');
		}
		else{
			//printf("printing digit... %d, from %d", digit, fixed);
...
//Min and Max saved for Plot Function
int32_t MinX, MaxX, MinY, MaxY;

//2 PX Border Sizes
#define TitleBuffer 32
#define PlotWidth 124
#define PlotHeight 124
#define PlotBorder 2
...
void ST7735_XYplotInit(char *title, int32_t minX, int32_t maxX, int32_t minY, int32_t maxY){
	...
	//cursor to begining of page
	ST7735_SetCursor(0, 0);
	
	//save mins and maxs for XYplot
	MinX = minX;
	MaxX = maxX;
	MinY = minY;
	MaxY = maxY;
	
	//print title -- doesn't handle wrapping
	ST7735_OutString(title);
	ST7735_OutChar('\n');
	
	//draw white rectangle
	ST7735_FillRect(PlotBorder, TitleBuffer + PlotBorder, PlotWidth, PlotHeight, ST7735_WHITE);
	
	//x axis
	//ST7735_DrawFastHLine(ST7735_TFTHEIGHT - 1, 0, ST7735_TFTWIDTH - 2, ST7735_GREEN);
	//y axis
	//ST7735_DrawFastVLine(0, 1, ST7735_TFTHEIGHT - 2, ST7735_GREEN);
}
...
*/
			...
			//xMin maps to 0
			//xMax maps to ST7735_TFTWIDTH
uint32_t XScale(int32_t n){
	int32_t x = ((n - MinX) * PlotWidth)/(MaxX - MinX);
	return x + PlotBorder;
}	
			//YMin maps to 0
			//YMax maps to ST7735_TFTHEIGHT - 
uint32_t YScale(int32_t n){
	int32_t y = ((n - MaxY) * PlotHeight)/(MinY - MaxY);
	return y + PlotBorder + TitleBuffer;
}	
...
void ST7735_XYplot(uint32_t num, int32_t bufX[], int32_t bufY[]){
	...
	for(int i = 0; i < num; i++){
		...
		if( (x < MinX) || (x > MaxX) || (y < MinY) || (y > MaxY) ){
			...
			//scale for Sitronix Screen
			x = XScale(x);
			y = YScale(y);
			
			//draw a point
			ST7735_FillRect(x, y, 2, 2, ST7735_BLUE);
		}
	} // end for loop
}


void ST7735_XYplotVLine(int32_t x, int32_t y){
	// draw points in blue		
		if( (x < MinX) || (x > MaxX) || (y < MinY) || (y > MaxY) ){
			//out of bounds: do nothing
		}
			...
			//draw a line
			ST7735_DrawFastVLine(x, y, 158 - y, ST7735_BLUE);
		}
}
...
void ST7735_uDecOut0(uint32_t n){
	...
	while(divisor>10){
		if(remainder > divisor-1){
			ST7735_OutChar((remainder/divisor) + '0');
